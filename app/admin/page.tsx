  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 font-sans">
      ...
      <header className="bg-slate-900 text-white border-b border-slate-800 px-4 py-3 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="inline-flex items-center gap-1.5 text-xs text-slate-300 hover:text-white bg-slate-800 px-2.5 py-1 rounded-md transition-colors border border-slate-700"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>Voltar ao RadarParadesporto</span>
            </Link>

            <div className="flex items-center gap-2">
              <span className="font-bold text-sm">Painel de Moderação & Gestão</span>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400">
            <span>Aprovações entram ao vivo em tempo real</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-xl font-bold text-[#0f2a4a] tracking-tight">
              Curadoria de Iniciativas
            </h1>
            <p className="text-xs text-slate-600 mt-0.5">
              Gerencie submissões de organizações paradesportivas enviadas pela comunidade.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-md text-center">
              <span className="text-[11px] font-medium text-amber-800 block">Pendentes</span>
              <span className="text-lg font-bold text-amber-900">{pendentes.length}</span>
            </div>
            <div className="p-2.5 bg-emerald-50 border border-emerald-200 rounded-md text-center">
              <span className="text-[11px] font-medium text-emerald-800 block">Publicadas</span>
              <span className="text-lg font-bold text-emerald-900">{publicadas.length}</span>
            </div>
            <div className="p-2.5 bg-slate-100 border border-slate-200 rounded-md text-center">
              ...
              <span className="text-lg font-bold text-slate-800">{allIniciativas.length}</span>
            </div>
          </div>
        </div>
        ...
        {actionSuccessMsg && (
          <div className="bg-emerald-50 border border-emerald-300 text-emerald-900 px-3.5 py-2.5 rounded-md flex items-center gap-2 text-xs font-medium">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-3 flex flex-col sm:flex-row items-center justify-between gap-3">
          {/* Tabs por Status */}
          <div className="flex items-center gap-1 w-full sm:w-auto overflow-x-auto">
            ...
            >
              ...
              <span>Publicadas ({publicadas.length})</span>
            </button>
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1 bg-slate-50 border border-slate-300 rounded-md text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900"
            />
          </div>
        </div>
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            filteredList.map((item) => (
              <div
                key={item.id}
                className={`bg-white rounded-lg border p-5 transition-colors space-y-3 ${
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 pb-2.5 border-b border-slate-100">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="text-base font-bold text-slate-900">{item.nome}</h3>
                      {item.status === 'pendente' && (
                        <span className="text-[11px] font-semibold text-amber-800 bg-amber-100 px-2 py-0.5 rounded-md border border-amber-200">
                      ...
                      {item.status === 'rejeitado' && (
                        <span className="text-[11px] font-semibold text-red-800 bg-red-100 px-2 py-0.5 rounded-md border border-red-200">
                          Rejeitado
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {item.status !== 'publicado' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-semibold px-3 py-1.5 rounded-md transition-colors"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span>Aprovar</span>
                      </button>
                    ...
                    {item.status === 'pendente' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 bg-slate-100 hover:bg-red-50 text-slate-700 hover:text-red-700 text-xs font-medium px-2.5 py-1.5 rounded-md border border-slate-300 transition-colors"
                      >
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-1 text-slate-700 hover:text-slate-900 bg-slate-100 px-2.5 py-1.5 rounded-md text-xs font-medium border border-slate-300 transition-colors"
                    >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.esportes.map((esp) => (
                        <span
                          key={esp}
                          className="bg-blue-50 text-blue-900 font-medium px-2 py-0.5 rounded-md border border-blue-200 text-xs"
                        >
                          {esp}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.deficienciasAtendidas.map((def) => (
                        <span
                          key={def}
                          className="bg-slate-100 text-slate-700 font-medium px-2 py-0.5 rounded-md border border-slate-200 text-xs"
                        >
                          {def}
                        </span>
                      ))}
                ...
                {/* Canais de Contato */}
                <div className="pt-2.5 border-t border-slate-100 flex flex-wrap items-center gap-3 text-xs text-slate-600">
                  {item.whatsapp && (
                  ...
                  {item.instagram && (
                    <span className="inline-flex items-center gap-1 font-medium text-slate-700">
                      <InstagramIcon className="w-3.5 h-3.5 text-slate-500" /> {item.instagram}
                    </span>
                  )}
                  {item.email && (
                    <span className="inline-flex items-center gap-1">
                  ...
                  {item.site && (
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-slate-700 font-medium underline"
                    >
            ...
            <div className="py-10 text-center bg-white rounded-lg border border-slate-200 p-6">
              <h3 className="text-xs font-bold text-slate-700">Nenhum registro encontrado</h3>
